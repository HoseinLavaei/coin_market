...
class RamzinexProvider:
    ...
    async def _fetch_pairs_map(cls) -> dict[tuple[str, str], int]:
        """Fetch pairs and build market map: (quote_symbol, base_symbol) -> pair_id."""
        try:
    ...
    def _parse_otc_market(cls, market: dict, quote: Quote, bases: list[Base]) -> Coin | None:
        ...
        return Coin(
            ...
            base=base,
            _buy_price=Decimal(str(buy_price)) / 10,
            _sell_price=Decimal(str(sell_price)) / 10,
            buy_fee=Decimal(0),
            sell_fee=Decimal(0),
            quote=quote,
    ...
    def _build_order_list(cls, entries: list, q: Quote, b: Base, now: datetime.datetime) -> list[Order]:
        ...
        for entry in entries:
            ...
            try:
                ...
                amount = cls._clean_number(entry[1])  # amount is already in base currency
